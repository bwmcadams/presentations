...
\begin{itemizecodeframe}
    ...
    \item One Geoindex allowed per collection
    ...
    \item I loaded the publicly available GTFS data for EPTA's Rail and Bus services.
    \item Quick \& Dirty Python script to create the index:
        \lstinputlisting[language=Python]{code/geospatial_idx.py}
    \pagebreak
    \item What are the 5 nearest SEPTA stops to our current location (`39.946332, -75.144009')?
        \lstinputlisting[language=JavaScript]{code/geospatial.js}
    \pagebreak
    \item It is possible to use a secondary key as well to further filter.  The GTFS data is designed more relationally, but if it had a "train" or "bus" field...
        \lstinputlisting[language=JavaScript]{code/geospatial_sec.js}
    \pagebreak
    \item `\$near' queries can also take  `\$maxDistance' parameter which limits the search area. 
        \lstinputlisting[language=JavaScript]{code/geospatial_max.js}
    \item `\$near' queries just get you the `closest'.  You can use `\$within' to specify a shape such as `\$box' (rectangle), `\$center' (circle), `\$polygon' (concave and convex polygons).
    \pagebreak
    \item By default, treats things like a Map (flat)... but 1.8 has support for treating things like a sphere (e.g. the earth)
    \item In production use at Foursquare \& Wordsquared (Formerly Scrabb.ly)
    \item It doesn't HAVE To be map coordinates.  Scrabb.ly tracks the coordinates of infinite scrabble boards and the tiles' relation to 0, 0
\end{itemizecodeframe}
