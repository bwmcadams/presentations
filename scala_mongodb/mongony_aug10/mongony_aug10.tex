...
\begin{itemizecodeframe}
    \frametitle{Classes in Scala}
    \item Classes are similar to most languages.
    \item Made up of properties and methods.
    \item An instance of a class is created with the ``new'' keyword.
    \item No multiple inheritance, but can implement traits.
    \item Constructors are part of the class definition, rather than separate methods.
    \item Overloaded constructors are allowed, but must call another constructor \emph{before any other code is executed}. 
        \lstinputlisting{code/class.scala}
\end{itemizecodeframe}

\begin{itemizecodeframe}
    \frametitle{Traits in Scala}
    \item Easiest way to think of a Trait is an interface with concrete declarations; allows abstract and concrete definitions.
    \item Traits can \textbf{\emph{NOT}} be directly instantiated.
    \item Traits are stackable - multiple traits can call one another for specific functionality.
    \item Traits can be used as ``mixins'' including being added at class instantiation to change functionality.
    \lstinputlisting{code/trait.scala}
    
\end{itemizecodeframe}

\begin{itemizecodeframe}
    \frametitle{Scala's Object Type}
    \item Scala does not support static method declaration in classes; any method declared in an ``object'' is static.
    \item Scala objects act as ``companions'' - if an object and class with the same name exist, they complement each other.
    \item Scala objects are a system managed singleton - only one instance of the object ever exists (From Java's view an ``object'' is actually <Name>\$)
    \item Companion objects are often used as factories to prevent/proxy direct class instantiation.
    \lstinputlisting{code/object.scala}
\end{itemizecodeframe}

\begin{itemizecodeframe}
    \frametitle{User Defined Operators}
    \item Scala allows the definition of operators...
    \item<2-> Not technically operator overloading, as the JVM doesn't have operators - they're language built-ins in Java, etc.  
    \item<3-> In Scala, there are no built-in operators.  Some are predefined for sanity (like \texttt{+}, \texttt{-}, \texttt{/} and \texttt{*} on Numeric types)  but operators are just methods.
    \item<3-> Scala allows any operator to be defined by the user - including some special ones like unaries (\texttt{+foo}, \texttt{-foo}).
    \item<4-> Syntactic Sugar: To facilitate statements like \texttt{foo + bar} Scala allows methods to be called without the \texttt{.} or parentheses\ldots Useful for DSLs and fluid syntaxes!
...
\begin{itemizecodeframe}
    ...
    \item A few crucial Scala concepts which depend upon FP (and Scala programmers delight in)
        \begin{itemize}
            ...
            \item Useful tools like \texttt{group}, \texttt{foreach} and \texttt{map}
        \end{itemize}
        \lstinputlisting{code/fp.scala}
\end{itemizecodeframe}
